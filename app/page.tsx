      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {botStrategies.map((bot) => (
            <Link href={`/bot-strategies/${bot.id}`} key={bot.id} className="bg-foreground p-4 rounded-lg">
              <div>
                <h2 className="text-xl font-bold text-[#a5a6b5]">{bot.name}</h2>
                <p className="text-gray-500 text-sm">Created {moment(bot.created_at).fromNow()}</p>
                <p className="text-lg font-bold mt-2 flex items-center gap-2">
                  ...
                  {bot.profit < 0 && (
                    <span className="text-red-500">-{bot.profit}%</span>
                  )}
                  {bot.profit === 0 && (
                ...
                <p className="text-gray-400 text-sm pt-2">
                  ...
                  {bot.totalTrades > 0 && (
                    <span>Total trades: <b className="text-white">{bot.totalTrades}</b></span>
                  )}
                </p>
              </div>
            </Link>
